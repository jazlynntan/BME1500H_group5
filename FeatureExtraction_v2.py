...
def load_spiketrain(filepath, to_plot = False):
    """
    Loads spike train data from a file and visualizes it.
    
    Args:
    - filepath: Path to the file containing spike train data.
    - to_plot (optional): If True, plots the spike train data.

    Returns:
    - analogsignal: Raw analog waveform.
    - spike_times: Array of spike timings.
    - sampling_frequency: Sampling frequency of the analog signal.
    - time: Time vector.
    """
...
def get_firing_rate(spike_times, recording, sampling_rate):
    """
    Calculates firing rate across full recording
    
    Args:
    - spike_times: array of spike times (in seconds)
    - recording: raw analog signal
    - sampling rate: frequency in Hz at which the data was sampled
...
def calculate_instantaneous_firing_rate(spike_times, recording, sampling_rate, step_size, window):
    """Calculates instantaneous firing rate."""
...
def get_ifr_metrics(ifr):
    """Calculates metrics for instantaneous firing rate."""
...
def get_mean_amplitude(spike_times, recording, sampling_rate, window):
    """
    Calculates mean amplitude defined as peak-trough change 
    
    Args:
    - spike_times: array of spike times (in seconds)
    - recording: raw analog signal
    - sampling rate: frequency in Hz at which the data was sampled
    - window: number of samples on each side of spike peak to consider for one spike 

    Returns:
    - mean_amplitude: mean amplitude
    - amplitude_ls: list of all spike amplitudes
    """
    amplitude_ls = []
...
def get_mean_amplitude2(spike_times, recording, sampling_rate, window):
    """
    Calculates mean amplitude defined as larger of the peak or trough - baseline as estimated via mean of spike segment
...
def get_ecephys_waveform_metrics(spike_times, recording, window, sampling_rate):
    ...
    Returns:
    - waveform_duration: see WaveformMetrics
    - waveform_halfwidth: see WaveformMetrics
    - waveform_PT_ratio: see WaveformMetrics
    - waveform_TP_time: trough to peak time
    - waveform_positive_spiking: whether peak or trough occurs first
    """
...
def get_ISI_metrics(spike_times):
    """
    Calculates interspike interval metrics.
    ...
    Returns:
    - ISI_cv: coefficient of variation of ISI distribution
    - ISI_skew: skew of ISI distribution
    - ISI_kurtosis: kurtosis of ISI distribution
    - ISI_mean: mean ISI
    - ISI_mode: peaks in the ISI distribution
    """
...
def burst_detection_neuroexplorer(spike_times, 
    ...
    """
    Detects bursts in spike train data.
    ...
    - recording: raw analog signal
    - sampling rate: frequency in Hz at which the data was sampled
    - min_surprise: param for Poisson surprise method
    - min_numspikes: minimum number of spikes in burst to be considered a burst event
    
    Returns:
    - burst_dict: dictionary of burst features for get_burst_metrics
...
def get_burst_metrics(burst_dict, spike_times):
    """
    Calculates burst metrics.
    
    Args:
    - burst_dict: 
        - burst_numspikes: number of spikes in burst
        - burst_surprise: surprise value of burst
        - burst_start_spike: spike index that starts the burst
        - burst_end_spike: spike index that ends the burst
    - spike_times: array of spike times (in seconds)
    
    Returns:
    - num_bursts: number of burst events
    - mean_surprise: mean surprise value of bursts
    - burst_index: number of spikes in burst out of all spikes
    - mean_burst_duration: mean length of burst (in seconds)
    - mean_interburst_duration: mean length of interburst duration (in seconds)
    - var_burst_duration: variance of burst length
    - var_interburst_duration: variance of interburst length
    """
...
def get_burst_index(spike_times):
    ...
    uses median ISI as proxy for mode ISI due to multimodal distributions
...
def get_synchrony_features2(spike_times, 
    ...
    """
    Calculates synchrony features using autocorrelation
    ...
    - time_bin_size
    - max_lag_time
    - significance_level: to identify significant frequencies
    - to_plot (optional): if True, plots autocorrelogram and periodogram
    
    Returns:
    - max_peak_freq: frequency that has highest magnitude
    - freq_peaks: significant frequencies in recording
    - peak_bands: frequency bands present in recording
    - freq_magnitude: magnitude of significant frequencies
    """
